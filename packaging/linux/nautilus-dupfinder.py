#!/usr/bin/env python3
"""
Nautilus extension for DupFinder
...
class DupFinderExtension(GObject.GObject, Nautilus.MenuProvider):
    def __init__(self):
        super().__init__()
        
    def _run_dupfinder(self, paths):
        """Launch DupFinder with selected paths"""
        try:
            args = ['dupfinder', '--scan'] + paths
            subprocess.Popen(args)
        except Exception as e:
            print(f"Error launching DupFinder: {e}")
    
    def _run_quick_scan(self, paths):
        """Launch DupFinder quick scan"""
        try:
            args = ['dupfinder', '--quick-scan'] + paths
            subprocess.Popen(args)
        except Exception as e:
            print(f"Error launching DupFinder: {e}")
    ...
    def get_file_items(self, *args):
        ...
        item = Nautilus.MenuItem(
            name='DupFinderExtension::FindDuplicates',
            label='Find Duplicates with DupFinder',
        ...
        quick_item = Nautilus.MenuItem(
            name='DupFinderExtension::QuickScan',
    ...
    def get_background_items(self, *args):
        ...
        item = Nautilus.MenuItem(
            name='DupFinderExtension::FindDuplicatesHere',
