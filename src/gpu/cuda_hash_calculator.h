...
// CUDA SHA-256 hash calculator
class CUDAHashCalculator : public GPUHashCalculator {
public:
    CUDAHashCalculator();
    ~CUDAHashCalculator() override;

    // GPUHashCalculator interface
    bool initialize() override;
    std::vector<uint8_t> computeHash(const std::vector<uint8_t>& data) override;
    std::vector<std::vector<uint8_t>> computeHashes(
        const std::vector<std::vector<uint8_t>>& dataBlocks) override;
